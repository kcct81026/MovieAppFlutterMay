...
class MoiveDetailsPage extends StatefulWidget {
  final int movieId;
  ...
  @override
  State<MoiveDetailsPage> createState() => _MoiveDetailsPageState();
}

class _MoiveDetailsPageState extends State<MoiveDetailsPage> {
  ...
  void initState() {
    ...
    ?.then((creditList){
      setState(() {
        ...
        mCreatorsList = creditList.where((creator) => creator.isCreator()).toList();
      });
    });
            ...
            SliverList(
              delegate: SliverChildListDelegate([
                Container(
                  ...
                  child: TrailerSectionView(movieVO!),
                ),
                ...
                Container(
                  margin: EdgeInsets.symmetric(horizontal: MARGIN_MEDIUM_2),
                  child:  AboutFilmSectionView(movieVO!),
                ),
                SizedBox(height: MARGIN_MEDIUM_2,),
                ActorAndCreatorsSectionView(
                    CREATOR_TITLE, SEE_MORE_SHOWCASES_TITLE, mCreatorsList ?? []),
...
void _navigateToMovieDetailsScreen(BuildContext context, int movieId) {
  Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => MoiveDetailsPage(movieId))
  );
}
